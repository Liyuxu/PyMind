...
class Graph(QGraphicsScene):
    ...
    def messageDialog(self):
        # 核心功能代码就两行，可以加到需要的地方
        msg_box = QMessageBox(QMessageBox.Warning, 'Warning', '当前选中为"根主题"!')
        msg_box.exec_()
    ...
    def textColor(self):
        ...
        dialog = QColorDialog()
        dialog.setWindowTitle('Set textColor')
    ...
    def textFont(self):
        ...
        dialog = QFontDialog()
        dialog.setWindowTitle('Set textFont')
        ...
        self.messageShow.emit('Info: change text font !')
    ...
    def contextMenuEvent(self, e):
        ...
        if selectedItem and not self.m_editingMode:
            ...
            # Set Color
            set_color_action = QAction('背景颜色', self)
            set_color_action.triggered.connect(self.nodeColor)
            rightclick_menu.addAction(set_color_action)

            # Set Text Color
            set_textColor_action = QAction('文字颜色', self)
            set_textColor_action.triggered.connect(self.textColor)
            rightclick_menu.addAction(set_textColor_action)

            # Set Text Font
            set_textFont_action = QAction('字体', self)
            set_textFont_action.triggered.connect(self.textFont)
            rightclick_menu.addAction(set_textFont_action)

            rightclick_menu.exec(QCursor().pos())
        elif self.m_editingMode:
            pass
            # set_font_action = QAction('Set Font', self)
            # set_font_action_action.triggered.connect(self.fontSet)
    ...
    def node_(self, node, father): 
        ET.SubElement(father,
            'node', {
                ...
                'm_textFont': str(node.m_textFont)
            })
    ...
    def readContentFromXmlFile(self, filename):
        ...
        for node_attr in root:
            ...
            print('attr: ', attr['m_textFont'], QFont(attr['m_textFont']), QFont('黑体', 16))
            #将xml中的数据存入节点中
            node.x = float(attr['x'])
            node.y = float(attr['y'])
            ...
            node.m_textFont = QFont(attr['m_textFont'])
            #node.m_textFont = QFont(attr['font-family'], attr['font-size'][:-3])
